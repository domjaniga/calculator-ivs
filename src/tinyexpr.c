    ...
    do {
        ...
        if ((s->next[0] >= '0' && s->next[0] <= '9') || s->next[0] == '.') {
            ...
            if (isalpha(s->next[0])) {
                ...
                switch (s->next++[0]) {
                    case '+': s->type = TOK_INFIX; s->function = lib_add; break;
                    case '-': s->type = TOK_INFIX; s->function = lib_subtract; break;
                    case '*': s->type = TOK_INFIX; s->function = lib_multiply; break;
                    case '/': s->type = TOK_INFIX; s->function = lib_divide; break;
                    case '^': s->type = TOK_INFIX; s->function = lib_power; break;
...
static te_expr *power(state *s) {
    /* <power>     =    {("-" | "+")} <base> */
    int sign = 1;
    while (s->type == TOK_INFIX && (s->function == lib_add || s->function == lib_subtract)) {
        if (s->function == lib_subtract) sign = -sign;
        next_token(s);
    }
...
static te_expr *factor(state *s) {
    ...
    while (s->type == TOK_INFIX && (s->function == lib_power)) {
...
static te_expr *term(state *s) {
    ...
    while (s->type == TOK_INFIX && (s->function == lib_multiply || s->function == lib_divide || s->function == fmod)) {
        te_fun2 t = s->function;
        next_token(s);
        te_expr *f = factor(s);
